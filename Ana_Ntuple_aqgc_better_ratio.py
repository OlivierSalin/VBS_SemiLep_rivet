...
Complement_path=f"/Models/Test/Ratio02/{name_plt}/"
...
aQGC_models_name= ["Aqgc_Eboli","aqgc_Eboli_Run3","aqgc_new"]
...
def plot_histograms2(desired_num_bins, root_files_info, output_plot, aQGC_models_name, key_name, Xsec_model, normalize_to_xsec=False, tree_name="Merged"):
    ...
    for parameter_to_plot in keys:
        ...
        for idx, (legend_name, file_path) in enumerate(root_files_info.items()):
            ...
            scale_factor = Xsec_model[legend_name] if normalize_to_xsec else 1.0
            histogram = ROOT.TH1D(f"histogram_{i}", "title", desired_num_bins, min_hist_, max_hist_)
            tree.Draw(parameter_to_plot + f">>histogram_{i}", "", "norm")
            histogram.Scale(scale_factor)
            ...
            if i == 0 and histogram.GetEntries() > 0:
                reference_hist = histogram.Clone("reference_hist")
                values_bins_hist["Ref_hist"]=[histogram.GetBinContent(i) for i in range(1,histogram.GetNbinsX()+1)]
            else:
                other_hists.append(histogram.Clone(f"hist_{i}"))
                values_bins_hist[f"Hist_{i}"]=[histogram.GetBinContent(i) for i in range(1,histogram.GetNbinsX()+1)]
            root_file.Close()
            i += 1

        if hs.GetNhists() > 0:
            print(f"Creating plot for {param_name} for {key_name_title}")
            canvas.Divide(1, 2)
            canvas.cd(1)
            ...
            print("Values bins hist: ", values_bins_hist)
            print(f"Reference histogram: {reference_hist.GetEntries()}")
            
            hist_ref = other_hists[0]
            reference_hist = hist_ref
            print(f"Reference histogram: {reference_hist.GetEntries()}")
            print(f"Other histograms: {other_hists}")
            
            ratio_hist = hist_ref
            print(f"Ratio histogram: {ratio_hist}")
            print(f"Ratio histogram: {ratio_hist.GetEntries()}")
            ratio_hist.Reset()
            
            for j in range(1,len(model_names)):
                for i in range(1, reference_hist.GetNbinsX() + 1):
                    print(f"Bin: {i}")
                    print(f"Reference histo: {reference_hist.GetNbinsX()}")
                    ref_value = reference_hist.GetBinContent(i)
                    print(f"Reference value: {ref_value}")
                    histo=other_hists[j-1]
                    #print(f"Reference value: {ref_value}")
                    #print(f"Reference histo: {histo.GetBinContent(i)}")
                    if ref_value == 0:
                        ratio_hist.SetBinContent(i, 0)
                        ratio_hist.SetBinError(i, 0)
                    else:
                        ratio_value = histo.GetBinContent(i) / ref_value
                        ratio_error = histo.GetBinError(i) / ref_value
                        ratio_hist.SetBinContent(i, ratio_value)
        ...
        for op in all_op_plot:
            ...
            for order_EFT in order:
                for index, name_model in enumerate(aQGC_models_name):
                    ...
                    if len(aQGC_models_name)>1 and aQGC_models_name[0] == aQGC_models_name[1]:
