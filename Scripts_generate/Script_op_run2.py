...
parser.add_option("--nb_events", default = 2000)
...
def find_prod_dec_and_dir_tres(conf, type_MC=None):
    ...
    if conf.startswith("user."):
        ...
        elif "DynScale" in type_MC or "dynsclae" in type_MC:
            base_dir = f"{base_path}/13p0/DynScale/{prod_dec}/"
        elif "dyn_nocut" in type_MC or "Dyn_nocut" in type_MC:
            base_dir = f"{base_path}/13p0Alt/dyn_nocut/{prod_dec}/"
        elif "sm_nocut" in type_MC or "sm_nocut" in type_MC:
            base_dir = f"{base_path}/13p0Alt/sm_nocut/{prod_dec}/"
        elif "alt_nocut" in type_MC or "alt_nocut" in type_MC:
            base_dir = f"{base_path}/13p0Alt/alt_nocut/{prod_dec}/"
...
if __name__ == "__main__":
    with Pool() as p:
        ...
        proc_decays_tuple = [
            (process, decay) for process, decay in itertools.product(processes, decays)
            if f"{process}_{decay}" in valid_combinaison_SM
        ]
        print("Filtered proc_decays_tuple: ", proc_decays_tuple)
        ...
        for decay in decays:
            ...
            if proc_decay in valid_combinaison_SM:
                for Gen in Generator_:
                    ...
                    Conf= f"user.osalin.MadGraph_{process}_{decay}_{order}_{Gen}"
                    print("Conf: ", Conf)
                    ...
                    path_hist_base ="/exp/atlas/salin/ATLAS/VBS_mc/eft_files/Histograms/SM_prod/SM_Run2_new/MCprod/Validation/SM/"
                    path_hist= path_hist_base + f"/{process}_{decay}/{opts.name_copy}/"
                    if not os.path.exists(path_hist):
                        os.makedirs(path_hist)
                    else:
                        print("Warning: directory already exists, skipping.")
                        timestamp = time.strftime("%m%d-%H%M")
                        path_hist = path_hist + f"/Already_exist/{opts.name_copy}_{timestamp}/{process}_{decay}/Run2/"
                        os.makedirs(path_hist)
