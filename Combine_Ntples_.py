...
def combine_ntuples_jetAlgo(dir_details, Processes, Decays, Operators,output_dir,truth=False,aQGC=False):
    ...
    operator_dir = output_dir + "Operators/"
    ...
    # Combine root files and cutflows per operator
    for op in Operators:
        ntuple_files_op = [value + "/ntuple_rivet.root" for key, value in paths.items() if op in key]
        if ntuple_files_op:
            subprocess.run(["hadd", "-f", operator_dir + f"/{op}_ntuple_rivet.root"] + ntuple_files_op)
            path_cutflow_op = [value for key, value in paths.items() if op in key]
            merge_cutflows(path_cutflow_op, operator_dir, "Combined", op)
