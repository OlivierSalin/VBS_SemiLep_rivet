    ...
    for process, decay, op in [(p, d, o) for p in processes for d in decays for o in all_op_plot]:
        ...
        for name_model_ in Run_model_name:
            ...
            if glob.glob(xsec_path):
                print(f"Reading cross section from: {xsec_path}")
                xsec = float(open(xsec_path, "r").readline())
            else:
                print(f"Cross section file not found at: {xsec_path}, trying alternative method.")
            ...
            if xsec is not None:
                X_section[key] = xsec
                SumWeights[key] = sumW
                if "Rwg" not in name_model_ and "reweight" not in name_model_.lower():
...
def main():
    ...
    variables_plot = ["merged_VlepVhad_mass",'merged_cos_theta_star','merged_tagjets_delta_eta', "merged_CS_V_cos_theta","merged_fjet_mass","merged_fjet_pt", "merged_Vlep"]
    ...
    Special_name = "/Reweighting/INT/Validation"


    Complement_path = f"INT/Validation/Plots/test/test04/"
    ...
    aQGC_models = ["EFTDec_INT","Rwg_INT"]
    #aQGC_models = ["EFTDec_INT"]
    
    for op_rwg in operators_rwg:
        if op_rwg in ["FM", "FT","FS"]:
            eftdec_ops = [op for op in all_operators if op.startswith(op_rwg[:2])]
            print(f"EFTDec ops for {op_rwg}: {eftdec_ops}")
        else:
