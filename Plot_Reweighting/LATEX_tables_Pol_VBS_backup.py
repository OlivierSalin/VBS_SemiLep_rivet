...
def generate_latex(df, caption, label, process_title_map, process_order, is_fiducial=False):
    ...
    cols_per_proc = 3 if is_fiducial else 2
    ...
    for _ in process_order:
        header_sub.append(r"\textbf{$\sigma$ [fb]}")
    ...
    for op in operators:
        ...
        for proc in process_order:
            ...
            row_cells.append(fmt_xsec(unpol))
            row_cells.append("") # pol frac
            if is_fiducial:
        ...
        for _, (pol_label, pol_col, acc_col) in enumerate(pol_rows):
            ...
            for proc in process_order:
                ...
                # xsec_pol = unpol * frac_pct / 100.0
                
                row_cells.append("") # Empty xsec for pol rows
...
path_to_tables_csv =f"{base_path}/acceptance/testGood"
