...
def extract_cross_sections_and_uncertainties(histograms_by_operator, xsec_dict, eff_dict, output_prefix):
    with open(f"{output_prefix}.txt", "w") as xsec_txt, open(f"{output_prefix}_uncertainty.txt", "w") as unc_txt:
        for op_name, hist in histograms_by_operator.items():
            error = c_double(0.0)
            integral = hist.IntegralAndError(0, hist.GetNbinsX() + 1, error)

            xsec_gen = xsec_dict.get(op_name, 1.0)
            eff_sr = eff_dict.get(op_name, 1.0)

            xsec_scaled = xsec_gen * eff_sr
            unc_scaled = xsec_gen * error.value / integral if integral > 0 else 0.0

            print(f"{op_name}: Cross-section = {xsec_scaled:.4f} fb, Uncertainty = {unc_scaled:.4f} fb (eff_sr = {eff_sr:.6f})")
            xsec_txt.write(f"{op_name}: {xsec_scaled:.6f}\n")
            unc_txt.write(f"{op_name}: {unc_scaled:.6f}\n")
...
def main():
    ...
    Complement_path = f"Polarisation/Test/test02/Reweight_Cross/Cross_term_EFTDec_Pola/"
    ...
    aQGC_models = ["EFTDec_Madspin","Reweighthel_ignore_Polarisation"]
    aQGC_models = ["EFTDec_Madspin","EFTDec_Polarisation","Reweighting_hel_ignore_Polarisation"]
    #aQGC_models = ["EFTDec_Polarisation","Reweighthel_ignore_Polarisation"]
    
    types = ["EFTDec_Madspin", "Reweighting_hel_ignore_Polarisation"]
    ...
    for op_rwg in operators_rwg:
        ...
        Root_paths, X_section, SumW = find_files_and_Xsections_sumW(
            processes, decays, all_ops_both, op_rwg, order_, types, base_dir, Special_name
        )

        for type_MC in types:
            histograms_by_operator = {}
            xsec_dict = {}
            eff_dict = {}

            for key, path in Root_paths.items():
                if type_MC not in key:
                    continue
                parts = key.split('_')
                process, decay, op, order_EFT = parts[:4]
                op_key = f"{process}_{decay}_{op}_{order_EFT}"

                root_file = ROOT.TFile(path)
                tree = root_file.Get("Merged")
                tree_before = root_file.Get("Bef_cut")
                if not tree or not tree_before:
                    continue

                w_before = sum(event.EventWeight for event in tree_before)
                w_after = sum(event.EventWeight for event in tree)
                eff_sr = w_after / w_before if w_before > 0 else 0.0

                if any(keyword in type_MC for keyword in ["Reweight", "rwg", "reweight", "wg"]):
                    weight_branch = f"EventWeight_{op.lower()}_{order_EFT.lower()}"
                else:
                    weight_branch = "EventWeight"

                hist_name = f"hist_{op_key}"
                ROOT.gROOT.cd()
                tree.Draw(f"0>>{hist_name}(1000, 0, 5000)", weight_branch, "goff")
                hist = ROOT.gDirectory.Get(hist_name)
                if not hist:
                    print(f"Warning: Failed to retrieve histogram for {op_key}")
                    continue
                histograms_by_operator[op_key] = hist
                if key in X_section:
                    xsec_dict[op_key] = X_section[key]
                eff_dict[op_key] = eff_sr

            if histograms_by_operator:
                extract_cross_sections_and_uncertainties(histograms_by_operator, xsec_dict, eff_dict, f"VBS_fromPlot_xsec_run2_{type_MC}")
