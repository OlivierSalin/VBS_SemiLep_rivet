...
all_ops_cat = ["FM0","FM2","FS0","FT0"]  # Reduced set for testing
...
#Type_MC_all= [ "EFTDec_polarisation", "Rwg_hel_ignore_polarisation", "Rwg_hel_aware_polarisation"]
        ...
        for op in all_ops_cat:
            if "madspin" in type_MC.lower():
                ...
                for pol in polarisations:
                    ...
                    print(f"[INFO] Found {len(matches)} matches for {conf} in {type_MC}")
...
if results:
    ...
    mixte_rows = []
    group_cols = [col for col in df.columns if col not in ["Polarisation", "xsec_fb", "uncertainty_fb"]]
    for keys, group in df.groupby(group_cols):
        lt = group[group["Polarisation"] == "LT"]
        tl = group[group["Polarisation"] == "TL"]
        if not lt.empty and not tl.empty:
            xsec_fb = float(lt["xsec_fb"].values[0]) + float(tl["xsec_fb"].values[0])
            unc_fb = np.sqrt(float(lt["uncertainty_fb"].values[0])**2 + float(tl["uncertainty_fb"].values[0])**2)
            row = dict(zip(group_cols, keys))
            row["Polarisation"] = "Mixte"
            row["xsec_fb"] = xsec_fb
            row["uncertainty_fb"] = unc_fb
            mixte_rows.append(row)
    if mixte_rows:
        df = pd.concat([df, pd.DataFrame(mixte_rows)], ignore_index=True)
    # Save with scientific notation (e.g., 3.62e-3) for all float columns
    float_cols = ["xsec_fb", "uncertainty_fb"]
    for col in float_cols:
        df[col] = df[col].apply(lambda x: np.format_float_scientific(x, precision=2, exp_digits=1))
        
    name= opts.name 
    output_excel = os.path.join(filepath_base, f"Tables/Excel/All_Polarisation_xsec_{name}.xlsx")
    os.makedirs(os.path.dirname(output_excel), exist_ok=True)
    df.to_excel(output_excel, index=False)
    print(f"[INFO] All results saved to {output_excel}")
