    ...
    for process, decay in proc_decays_tuple:
        ...
        for op in all_ops_cat:
            ...
            if "madspin" in type_MC.lower():
                ...
                # Only do the special config for FS/FT if both inv/xsec and FS/FT
                if (("inv" in type_MC.lower() or "xsec" in type_MC.lower()) and ("FS" in op_rwg or "FT" in op_rwg)):
...
if results:
    ...
    os.makedirs("Tables/Txt/test", exist_ok=True)
    ...
    for type_mc in df["Type_MC"].unique():
        ...
        outdir = f"./Tables/Uncertainty/Xsec/"
        ...
        # --- BEGIN: Write uncertainty TXT table ---
        unc_lines = []
        for order_eft in eft_orders:
            df_order = df.copy()
            if order_eft == "CROSS":
            ...
            for op in sorted(df_mc["Operator"].unique()):
                ...
                df_pol = df_op[df_op["Polarisation"].isin(pol_states)]
                pol_uncs = df_pol["uncertainty_fb"].values.astype(float)
