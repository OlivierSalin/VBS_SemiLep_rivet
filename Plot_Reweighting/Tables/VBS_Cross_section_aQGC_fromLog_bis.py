...
outdir_path= f"./Cross_section/Validation/Validation_bis/{opts.name}/"
        ...
        for process_decay in valid_combi:
            ...
            for order_eft in eft_orders:
                ...
                for op in sorted(df_mc["Operator"].unique()):
                    ...
                    # Filter polarisations for symmetric processes to avoid double counting in sums
                    if process in ["ZZ", "WpWp", "WmWm", "ssWW"]:
                        df_op = df_op[df_op["Polarisation"] != "TL"]
