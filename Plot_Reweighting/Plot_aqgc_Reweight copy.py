...
operators_rwg= ["FM","FS","FT","FSFM","FSFT","FMFT"]

all_ops_both = ["FS0", "FS1", "FS2"]
aQGC_models_name = ["rwg_fs_Nohel", "aqgc"]

Complement_path = f"Study_SpinCorrelation/Test/{opts.type_MC}/"
...
variables_plot = [
    ...
    "merged_Vlep_pt","merged_Vlep_mass", "merged_lepton1_pt"
]
...
# --- Plotting function ---
def plot_histograms2(desired_num_bins, root_files_info, output_plot, Process_name, Xsec_model,
                     norm_to_xsec=False, tree_name="Merged", weight_cut=None,perOp=True):
...
def compare_reweighting_vs_eftdec(processes, decays, order, all_operators, operators_rwg, base_dir, aQGC_models_name):
    types = ["Madspin", "NoSpin", "Decay_chain"]
    Special_name = "/Reweighting/Rwg_test/"
    for type_ in types:
        ...
        for process in processes:
            for decay in decays:
                for op_rwg in operators_rwg:
                    # Select EFTDec operators based on op_rwg logic
                    ...
                    for op in eftdec_ops:
                        Root_paths = {}
                        X_section = {}
                        # Reweighting path
                        rwg_path = construct_path(
                            base_dir + Special_name,
                            reweighting_cat,
                            process,
                            decay,
                            op_rwg,
                        ...
                        rwg_file = os.path.join(rwg_path, "ntuple_rivet.root")
                        rwg_xsec = os.path.join(rwg_path, "X_section_fb.txt")
                        # EFTDec path
                        eftdec_path = construct_path(
                            base_dir + Special_name,
                            eftdec_cat,
                            process,
                            decay,
                        ...
                        eftdec_file = os.path.join(eftdec_path, "ntuple_rivet.root")
                        eftdec_xsec = os.path.join(eftdec_path, "X_section_fb.txt")
                        # Only plot if both files exist
                        if os.path.exists(rwg_file) and os.path.exists(eftdec_file):
                            # Consistent legend/process_name format: {process}_{decay}_{op}_{order_EFT}
                            order_EFT = order[0]
                            legend_rwg = f"{process}_{decay}_{op_rwg}_{order_EFT}"
                            legend_eftdec = f"{process}_{decay}_{op}_{order_EFT}"
                            Root_paths[legend_rwg] = rwg_file
                            Root_paths[legend_eftdec] = eftdec_file
                            # Xsec
                            if os.path.exists(rwg_xsec):
                                with open(rwg_xsec) as f:
                                    X_section[legend_rwg] = float(f.readline().strip())
                            else:
                                X_section[legend_rwg] = 1.0
                            if os.path.exists(eftdec_xsec):
                                with open(eftdec_xsec) as f:
                                    X_section[legend_eftdec] = float(f.readline().strip())
                            else:
                                X_section[legend_eftdec] = 1.0
                            # Plot
                            outPlot = os.path.join(base_dir_plot, f"Comparison_{type_}", f"{op_rwg}_{op}")
                            os.makedirs(outPlot, exist_ok=True)
                            Process_name = legend_rwg  # Use the same format as plot_histograms2 expects
                            plot_histograms2(
                                int(opts.bins),
                                Root_paths,
                                outPlot,
...
def main():
    ...
    aQGC_models = aQGC_models_name

    Root_paths, X_section, SumW = find_files_and_Xsections_sumW(
        processes, decays, all_ops_both, order, aQGC_models, base_dir, Special_name
    )
    ...
    for process in processes:
        for decay in decays:
            ...
            outPlot = f"{base_dir_plot}/OP/"
            os.makedirs(outPlot, exist_ok=True)
            
            #plot_histograms2(num_bins, Root_paths_model, outPlot, Process_name, X_section, norm_xsec, tree_name="Merged", weight_cut=weight_cut, perOp=False)
            for op in all_op_plot:
                ...
                os.makedirs(outPlot, exist_ok=True)
                plot_histograms2(num_bins, Root_paths_model, outPlot, Process_name, X_section, norm_xsec, tree_name="Merged", weight_cut=weight_cut, perOp=True)
    # Add the new comparison logic
    compare_reweighting_vs_eftdec(processes, decays, order, all_operators, operators_rwg, base_dir, aQGC_models_name)
