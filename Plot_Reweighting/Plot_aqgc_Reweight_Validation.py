...
processes = ["ZZ","WpZ","WmZ"]
#processes = ["WpZ","WmZ","WmWm","WpWp","WpWm"]
decays = ["llqq","vvqq"]
decays = ["vvqq"]
order = ["QUAD"]
...
def find_files_and_Xsections_sumW(processes, decays, all_op_plot, op_rwg, order, Run_model_name, base_dir, Special_name):
    ...
    for process, decay, op in [(p, d, o) for p in processes for d in decays for o in all_op_plot]:
        ...
        for name_model_ in Run_model_name:
            ...
            #print(f"SumW: {sumW}")
            if xsec is not None and "Reweight" not in name_model_:
                X_section[key] = xsec
    ...
    for parameter_to_plot in keys:
        ...
        for legend_name, file_path in root_files_info.items():
            ...
            if any(keyword in model_name for keyword in ["Reweight",'rwg', 'reweight', 'wg']):
                ...
                #print(f"Using weight branch: {weight_branch_name}")
...
def main():
    ...
    all_operators=["FM0","FM1","FM2","FM3","FM7","FM9",
                "FS0","FS1",
                "FT0","FT2","FT5"]
    #all_operators = ["FM0"]
    ...
    variables_plot = [
        ...
        'merged_tagjets_delta_eta', 'merged_tagjets_m', 'merged_tagjets_pt', 'merged_Full_pt',"merged_fjet_mass", 'merged_fjet_pt',
    ...
    #aQGC_models = ["EFTDec_Madspin","EFTDec_Polarisation","Reweighthel_ignore_Polarisation"]
    #aQGC_models = ["EFTDec_Madspin","EFTDec_Polarisation","Reweighting_InvSqrtXsec"]
            ...
            for decay in decays:
                try:
                    ...
                    if not Root_paths_model:
                        ...
                        continue
                    #print(f"Root paths for {process} {decay}: {Root_paths_model}")
