...
def parse_options():
    ...
    parser.add_option("--CROSS", default="False")
...
def find_files_and_Xsections_sumW(processes, decays, all_op_plot, op_rwg, order, Run_model_name, base_dir, Special_name):
    ...
    for process, decay, op in [(p, d, o) for p in processes for d in decays for o in all_op_plot]:
        ...
        for name_model_ in Run_model_name:
            ...
            xsec = get_cross_section_cpodd(op, order_EFT, process, decay, name_model_)
...
def plot_histograms2(desired_num_bins,variables_plot, root_files_info, output_plot, Process_name, Xsec_model,
    ...
    for parameter_to_plot in keys:
        ...
        for legend_name, file_path in root_files_info.items():
            ...
            ##print(f"Process_op_order: {Process_op_order}, Xsec_model: {Xsec_model[Process_op_order]}")
            scale_factor = Xsec_model[Process_op_order] if norm_to_xsec else 1.0
            print(f"Processing {Process_op_order} with scale factor: {scale_factor}")
            ...
            if "EFTDec_Polarisation" in model_name:
...
def main():
    ...
    variables_plot = ['tagjets_delta_eta','tagjets_pt', 'tagjets_m']
    ...
    variables_plot = ["merged_VlepVhad_mass",'merged_tagjets_delta_eta','merged_tagjets_pt', 'merged_tagjets_m',"merged_fjet_mass","merged_fjet_pt",
                      "merged_Vlep_mass","merged_Vlep_pt"]
    ...
    Special_name = "/Reweighting/MCRequest/Validation/"


    Complement_path = f"MCRequest/Validation/Plots/test/VBS_all/Plots/"
    base_dir_plot = f"/exp/atlas/salin/ATLAS/VBS_mc/Plots/Plot_Reweighting/{Complement_path}/"



    aQGC_models = ["Rwg_validation"]
    
    for op_rwg in operators_rwg:
        if op_rwg in ["FM","FS","FT"]:
            eftdec_ops = [op for op in all_operators if op.startswith(op_rwg) and not op.endswith("odd")]
            #print(f"IN Operator RWG: {op_rwg}, EFTDec ops: {eftdec_ops}")
        elif op_rwg in ["FModd","FTodd"]:
            eftdec_ops = [op for op in all_operators if op.startswith(op_rwg[:-3]) and op.endswith("odd")]
        else:
            eftdec_ops = all_operators
        print(f"Operator RWG: {op_rwg}, EFTDec ops: {eftdec_ops}")
        ...
        for process in processes:
            for decay in decays:
                try:
                    ...
                    if not Root_paths_model:
                        print(f"No ROOT paths for {process}_{decay}, skipping.")
                        continue
                    print(f"Root paths for {process} {decay}")
                    
                    # aggregate plot (skip on failure)
                    try:
                        plot_histograms2(num_bins, variables_plot, Root_paths_model, outPlot, Process_name, X_section,
                                         norm_xsec, tree_name=tree_name, perOp=False, op_rwg=op_rwg)
                    except Exception as e:
                        print(f"Aggregate plotting failed for {Process_name}: {e} -- skipping aggregate.")
                    
                    # per-operator plots (skip failing operators)
                    for op in all_op_plot:
                        order_eft_ = "CROSS" if "vs" in op else order[0]
                        Process_name_op = f"{process}_{decay}_{op}_{order_eft_}"
                        Root_paths_model_op = {k: v for k, v in Root_paths.items() if f"{process}_{decay}_{op}" in k}
                        if not Root_paths_model_op:
                            print(f"No ROOT paths for {Process_name_op}, skipping operator.")
                            continue
                        try:
                            plot_histograms2(num_bins, variables_plot, Root_paths_model_op, outPlot, Process_name_op, X_section,
                                             norm_xsec, tree_name=tree_name, perOp=True, op_rwg=op_rwg)
                        except Exception as e:
                            print(f"Plotting failed for {Process_name_op}: {e} -- continuing with next operator.")
                except Exception as e:
                    print(f"Skipping {process}_{decay} due to unexpected error: {e}")
                    continue
