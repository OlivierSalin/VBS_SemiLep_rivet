...
namespace Rivet
{
    ...
    class WWjj_lvqq : public Analysis
    {
    public:
        /// Constructor
        RIVET_DEFAULT_ANALYSIS_CTOR(WWjj_lvqq);
        ...
        void init()
        {
            ...
            std::cout << "++++++assume .json for this WWjj_lvqq" << " is " << jsonfilestr << "\n";
            ...
            _cutflows_merged.addCutflow("WWjj_lvqq_selections", {
                                                                   "have_1_lep",
                                                                   "pt_lep1_MET_cut_80",
                                                                   "n_jets",
                                                                   "found_tag_jets",
            ...
            _cutflows_resolved.addCutflow("WWjj_lvqq_selections", {
                                                                    "have_1_lep",
                                                                    "pt_lep1_MET_cut_80",
                                                                     "n_jets",
                                                                     "found_tag_jets",
        ...
        void analyze(const Event &event)
        {
            ...
            //std::cout << "Len Cross section: " << event.crossSections().size() << std::endl;
            for (int i = 0; i < event.crossSections().size(); i++) {
                //std::cout << "Cross section: " << event.crossSections()[i] << std::endl;
            }
            int i_=1;
            for (auto& [key, value] : weightNameToIndex) {
                if (key.find("DYNSCALE") != std::string::npos || key.find("PDF303000") != std::string::npos) {
                    ...
                    //std::cout << "Cross section: " << event.crossSections()[i_] << std::endl;
                    i_++;
                    //printf("\n");
                }
            }
            ...
            for (const auto& [key, value] : weightNameToIndex) {
                if (key.find("DYNSCALE") != std::string::npos || key.find("PDF303000") != std::string::npos) {
                    std::string weightName = "Event_Weight_" + key;
                    std::replace(weightName.begin(), weightName.end(), '.', 'p');
                    
                    double weight = weights_mc[value];
                    if (weightMap.find(weightName) != weightMap.end()) {
                        *(weightMap[weightName]) = weight; // Update the variable via pointer
            ...
            FourMomentum lepton = lep1.mom();
    
            const MissingMomentum& METfinder = apply<MissingMomentum>(event, "METFinder");
            double MET_pt = METfinder.missingPt()/GeV;
            double MET_Et = METfinder.missingEt()/GeV;
            FourMomentum fourvec_MET = METfinder.missingMomentum();
    
            // Cuts on the pT of the leptons
            if (_docut==1 && (leptons[0].pT()<_jcuts["pt_lepton1"])) vetoEvent;
            if (_docut==1 && (MET_Et<80)) vetoEvent;
            ...
            FourMomentum fourvec_Vlep = lep1.mom() + fourvec_MET;
            ...
            FourMomentum fourvec_lep_minus_Vlep_rf__ = boost_Vlep_rf__.transform(lepton);
            ...
            if (n_fjets > 0)
            {
                ...
                if ((_docut == 1 && (fourvec_fjets.mass() >= _jcuts["m_fjet_WZ"][0] && fourvec_fjets.mass() <= _jcuts["m_fjet_WZ"][1])) || _docut == 0)
                {
                    ...
                    merged_lepton_pt = lep1.pT();
                    merged_lepton_eta = lep1.eta();

                    merged_MET_pt = MET_pt;
                    merged_MET_Et = MET_Et;
                    merged_MET_eta = fourvec_MET.eta();
                    merged_MET_phi = fourvec_MET.phi();
                    merged_lepton_MET_delta_pt = abs(lep1.pT()-fourvec_MET.pT());
                    merged_lepton_MET_delta_eta = abs(lep1.eta()-fourvec_MET.eta());
                    merged_lepton_MET_delta_phi = deltaPhi(lep1, fourvec_MET);
            ...
            resolvedRegion:
                ...
                resolved_lepton_pt = lep1.pT();
                resolved_lepton_eta = lep1.eta();

                resolved_MET_pt = MET_pt;
                resolved_MET_Et = MET_Et;
                resolved_MET_eta = fourvec_MET.eta();
                resolved_MET_phi = fourvec_MET.phi();
                resolved_lepton_MET_delta_pt = abs(lep1.pT()-fourvec_MET.pT());
                resolved_lepton_MET_delta_eta = abs(lep1.eta()-fourvec_MET.eta());
                resolved_lepton_MET_delta_phi = deltaPhi(lep1, fourvec_MET);
                ...
                resolved_DR_min_lepton_tagjets1 = std::min(deltaR(lep1, tag1_jet), deltaR(lep1, tag1_jet));
                resolved_DR_min_lepton_tagjets2 = std::min(deltaR(lep1, tag2_jet), deltaR(lep1, tag2_jet));
                resolved_DR_min_lepton_sigjets1 = std::min(deltaR(signal_jet1, lep1), deltaR(signal_jet1, lep1));
                resolved_DR_min_lepton_sigjets2 = std::min(deltaR(signal_jet2, lep1), deltaR(signal_jet2, lep1));
        ...
        double merged_MET_pt, merged_MET_Et, merged_MET_phi, merged_MET_eta,merged_lepton_pt,merged_lepton_eta,merged_lepton_phi;
        double merged_lepton_MET_delta_pt,merged_lepton_MET_delta_eta,merged_lepton_MET_delta_phi;
        double merged_Vlep_mass,merged_Vlep_pt, merged_Vlep_eta, merged_Vlep_phi, merged_Vlep_DR_tagjet1, merged_Vlep_DR_tagjet2;
        ...
        int merged_n_jets, merged_n_lepton_stable, merged_fjet_n;
        ...
        double resolved_n_jets,resolved_n_lepton_stable, resolved_tagjet1_pt, resolved_tagjet2_pt, resolved_tagjets_pt, resolved_tagjets_delta_pt;
        ...
        double resolved_MET_pt, resolved_MET_Et, resolved_MET_phi, resolved_MET_eta,resolved_lepton_pt,resolved_lepton_eta,resolved_lepton_phi;
        double resolved_lepton_MET_delta_pt,resolved_lepton_MET_delta_eta,resolved_lepton_MET_delta_phi;
        double resolved_Vlep_mass,resolved_Vlep_pt, resolved_Vlep_eta, resolved_Vlep_phi, resolved_Vlep_DR_tagjet1, resolved_Vlep_DR_tagjet2;
        ...
        double EventWeight_MUR0p5_MUF0p5_DYNSCALE1_PDF303000, EventWeight_MUR0p5_MUF0p5_DYNSCALE2_PDF303000, EventWeight_MUR0p5_MUF0p5_DYNSCALE3_PDF303000, EventWeight_MUR0p5_MUF0p5_DYNSCALE4_PDF303000, EventWeight_MUR0p5_MUF0p5_PDF303000;
        double EventWeight_MUR0p5_MUF1p0_DYNSCALE1_PDF303000, EventWeight_MUR0p5_MUF1p0_DYNSCALE2_PDF303000, EventWeight_MUR0p5_MUF1p0_DYNSCALE3_PDF303000, EventWeight_MUR0p5_MUF1p0_DYNSCALE4_PDF303000, EventWeight_MUR0p5_MUF1p0_PDF303000;
        double EventWeight_MUR1p0_MUF0p5_DYNSCALE1_PDF303000, EventWeight_MUR1p0_MUF0p5_DYNSCALE2_PDF303000, EventWeight_MUR1p0_MUF0p5_DYNSCALE3_PDF303000, EventWeight_MUR1p0_MUF0p5_DYNSCALE4_PDF303000, EventWeight_MUR1p0_MUF0p5_PDF303000;
        ...
        double EventWeight_MUR1p0_MUF2p0_DYNSCALE1_PDF303000, EventWeight_MUR1p0_MUF2p0_DYNSCALE2_PDF303000, EventWeight_MUR1p0_MUF2p0_DYNSCALE3_PDF303000, EventWeight_MUR1p0_MUF2p0_DYNSCALE4_PDF303000, EventWeight_MUR1p0_MUF2p0_PDF303000;
        double EventWeight_MUR2p0_MUF1p0_DYNSCALE1_PDF303000, EventWeight_MUR2p0_MUF1p0_DYNSCALE2_PDF303000, EventWeight_MUR2p0_MUF1p0_DYNSCALE3_PDF303000, EventWeight_MUR2p0_MUF1p0_DYNSCALE4_PDF303000, EventWeight_MUR2p0_MUF1p0_PDF303000;
        double EventWeight_MUR2p0_MUF2p0_DYNSCALE1_PDF303000, EventWeight_MUR2p0_MUF2p0_DYNSCALE2_PDF303000, EventWeight_MUR2p0_MUF2p0_DYNSCALE3_PDF303000, EventWeight_MUR2p0_MUF2p0_DYNSCALE4_PDF303000, EventWeight_MUR2p0_MUF2p0_PDF303000;
        ...
        // Populate weightMap with the variables
        std::map<std::string, double*> weightMap = {
            {"Event_Weight_MUR0p5_MUF0p5_DYNSCALE1_PDF303000", &EventWeight_MUR0p5_MUF0p5_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR0p5_MUF0p5_DYNSCALE2_PDF303000", &EventWeight_MUR0p5_MUF0p5_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR0p5_MUF0p5_DYNSCALE3_PDF303000", &EventWeight_MUR0p5_MUF0p5_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR0p5_MUF0p5_DYNSCALE4_PDF303000", &EventWeight_MUR0p5_MUF0p5_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR0p5_MUF0p5_PDF303000", &EventWeight_MUR0p5_MUF0p5_PDF303000},
            {"Event_Weight_MUR0p5_MUF1p0_DYNSCALE1_PDF303000", &EventWeight_MUR0p5_MUF1p0_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR0p5_MUF1p0_DYNSCALE2_PDF303000", &EventWeight_MUR0p5_MUF1p0_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR0p5_MUF1p0_DYNSCALE3_PDF303000", &EventWeight_MUR0p5_MUF1p0_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR0p5_MUF1p0_DYNSCALE4_PDF303000", &EventWeight_MUR0p5_MUF1p0_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR0p5_MUF1p0_PDF303000", &EventWeight_MUR0p5_MUF1p0_PDF303000},
            {"Event_Weight_MUR1p0_MUF0p5_DYNSCALE1_PDF303000", &EventWeight_MUR1p0_MUF0p5_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR1p0_MUF0p5_DYNSCALE2_PDF303000", &EventWeight_MUR1p0_MUF0p5_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR1p0_MUF0p5_DYNSCALE3_PDF303000", &EventWeight_MUR1p0_MUF0p5_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR1p0_MUF0p5_DYNSCALE4_PDF303000", &EventWeight_MUR1p0_MUF0p5_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR1p0_MUF0p5_PDF303000", &EventWeight_MUR1p0_MUF0p5_PDF303000},
            {"Event_Weight_MUR1p0_MUF1p0_DYNSCALE1_PDF303000", &EventWeight_MUR1p0_MUF1p0_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR1p0_MUF1p0_DYNSCALE2_PDF303000", &EventWeight_MUR1p0_MUF1p0_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR1p0_MUF1p0_DYNSCALE3_PDF303000", &EventWeight_MUR1p0_MUF1p0_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR1p0_MUF1p0_DYNSCALE4_PDF303000", &EventWeight_MUR1p0_MUF1p0_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR1p0_MUF1p0_PDF303000", &EventWeight_MUR1p0_MUF1p0_PDF303000},
            {"Event_Weight_MUR1p0_MUF2p0_DYNSCALE1_PDF303000", &EventWeight_MUR1p0_MUF2p0_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR1p0_MUF2p0_DYNSCALE2_PDF303000", &EventWeight_MUR1p0_MUF2p0_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR1p0_MUF2p0_DYNSCALE3_PDF303000", &EventWeight_MUR1p0_MUF2p0_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR1p0_MUF2p0_DYNSCALE4_PDF303000", &EventWeight_MUR1p0_MUF2p0_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR1p0_MUF2p0_PDF303000", &EventWeight_MUR1p0_MUF2p0_PDF303000},
            {"Event_Weight_MUR2p0_MUF1p0_DYNSCALE1_PDF303000", &EventWeight_MUR2p0_MUF1p0_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR2p0_MUF1p0_DYNSCALE2_PDF303000", &EventWeight_MUR2p0_MUF1p0_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR2p0_MUF1p0_DYNSCALE3_PDF303000", &EventWeight_MUR2p0_MUF1p0_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR2p0_MUF1p0_DYNSCALE4_PDF303000", &EventWeight_MUR2p0_MUF1p0_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR2p0_MUF1p0_PDF303000", &EventWeight_MUR2p0_MUF1p0_PDF303000},
            {"Event_Weight_MUR2p0_MUF2p0_DYNSCALE1_PDF303000", &EventWeight_MUR2p0_MUF2p0_DYNSCALE1_PDF303000},
            {"Event_Weight_MUR2p0_MUF2p0_DYNSCALE2_PDF303000", &EventWeight_MUR2p0_MUF2p0_DYNSCALE2_PDF303000},
            {"Event_Weight_MUR2p0_MUF2p0_DYNSCALE3_PDF303000", &EventWeight_MUR2p0_MUF2p0_DYNSCALE3_PDF303000},
            {"Event_Weight_MUR2p0_MUF2p0_DYNSCALE4_PDF303000", &EventWeight_MUR2p0_MUF2p0_DYNSCALE4_PDF303000},
            {"Event_Weight_MUR2p0_MUF2p0_PDF303000", &EventWeight_MUR2p0_MUF2p0_PDF303000}
        };
        ...
        std::map<std::string, double *> varMap = {
            ...
            {"merged_lepton_pt", &merged_lepton_pt},
            {"merged_lepton_eta", &merged_lepton_eta},
            {"merged_lepton_phi", &merged_lepton_phi},
            {"merged_MET_pt", &merged_MET_pt},
            {"merged_MET_Et", &merged_MET_Et},
            {"merged_MET_phi", &merged_MET_phi},
            {"merged_MET_eta", &merged_MET_eta},
            {"merged_lepton_MET_delta_pt", &merged_lepton_MET_delta_pt},
            {"merged_lepton_MET_delta_eta", &merged_lepton_MET_delta_eta},
            {"merged_lepton_MET_delta_phi", &merged_lepton_MET_delta_phi},
        ...
        std::map<std::string, double *> varMap_resolved = {
            ...
            {"resolved_lepton_pt", &resolved_lepton_pt},
            {"resolved_lepton_eta", &resolved_lepton_eta},
            {"resolved_lepton_phi", &resolved_lepton_phi},
            {"resolved_MET_pt", &resolved_MET_pt},
            {"resolved_MET_Et", &resolved_MET_Et},
            {"resolved_MET_phi", &resolved_MET_phi},
            {"resolved_MET_eta", &resolved_MET_eta},
            {"resolved_lepton_MET_delta_pt", &resolved_lepton_MET_delta_pt},
            {"resolved_lepton_MET_delta_eta", &resolved_lepton_MET_delta_eta},
            {"resolved_lepton_MET_delta_phi", &resolved_lepton_MET_delta_phi},
    ...
    RIVET_DECLARE_PLUGIN(WWjj_lvqq);
