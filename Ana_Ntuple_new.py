...
def plot_histograms2(desired_num_bins, root_files_info, output_plot, aQGC_models_name, Xsec_model, normalize_to_xsec=False, tree_name="Merged"):
    ...
    def setup_canvas(parameter, model):
        canvas_name = f"canvas_{parameter}_{model}"
    ...
    for parameter_to_plot in keys:
        ...
        for model in aQGC_models_name:
            canvas = setup_canvas(parameter_to_plot, model)
            param_name = format_param_name(parameter_to_plot)
            key_name_title = f"{model}"
            if "Run3" not in model:
                ecm=""
            else:
                ecm="13.6 TeV"
            ecm = "13.6 TeV"
            sample_legend = f"Sample: {model} {ecm} (Merged)"
            ...
            for idx, (legend_name, file_path) in enumerate(root_files_info.items()):
                ...
                hs.Add(histogram)
                legend.AddEntry(histogram, f"{op}_{order_EFT}", "l")
                root_file.Close()
                i += 1
            ...
            if hs.GetNhists() > 0:
                ...
                canvas.Update()
                os.makedirs(output_plot + f"/{model}/", exist_ok=True)
                save_canvas(canvas, output_plot + f"/{model}/", parameter_to_plot)
