...
Complement_path="/SM_sample/MG_gen_prod/MC_request/MC_Prod/SM/All_processes/Run3_New_prod/"
...
Run_model_name = ["Run2_Old_prod","Run2"]
Run_model_name = ["Run3_New_prod"]
...
variables_plot= ['merged_VlepVhad_mass',"merged_VlepVhad_pt",'merged_fjet_pt','merged_fjet_mass','merged_tagjets_m',
                ...
                'resolved_VlepVhad_mass',"resolved_VlepVhad_pt","resolved_mjjj",'resolved_signal_jets_pt1','resolved_signal_jets_mass','resolved_tagjets_m',
...
valid_combination_colors = {
    "WZjj_llqq": ROOT.kRed,
    "ZZjj_llqq": ROOT.kBlue,
    "WZjj_vvqq": ROOT.kGreen + 3,
    "ZZjj_vvqq": ROOT.kViolet + 3,
    "WZjj_lvqq": ROOT.kOrange,
    "WWjj_lvqq": ROOT.kMagenta,
}
...
def plot_histograms2(desired_num_bins, root_files_info, output_plot, Process_name, Xsec_model, norm_to_xsec=False, tree_name="Merged"):
    ...
    for parameter_to_plot in keys:
        ...
        if "Run2" or "13p0" in model_name:
            ecm = "13.0 TeV"
        elif "Run3" or "13p6" in model_name:
            ecm = "13.6 TeV"
        #ecm = ""
        sample_legend = f"{ecm} ({tree_name})"
        ...
        for legend_name, file_path in root_files_info.items():
            ...
            scale_factor=1.0/(SumWeights_*eff_sr)
            scale_factor=1.0
           #  print(f"Scale factor: {scale_factor} for {legend_name}")
           ...
           #  print("fraction",Fraction_txt)
            ...
            elif "Sherpa" in model_name or "H7" in model_name:
                histogram.SetLineColor(ROOT.kBlue)
            histogram.SetLineColor(valid_combination_colors[proc_dec])
           ...
           #  print('Model name: ', model_name)
            ...
            legend.AddEntry(histogram, f"{proc_dec} SM (#sigma: {xsection:.2f} fb, fid: {eff_sr*100:.2f}%)", "l")
            root_file.Close()
            i += 1
...
for op in ["SM"]:
    ...
    for process in processes:
        for decay in decays:
            ...
            Process_name = f"{process}_{decay}_{op}"
