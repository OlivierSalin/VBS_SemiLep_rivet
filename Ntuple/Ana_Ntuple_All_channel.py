...
if not opts.linear:
    base_dir_plot = f"/exp/atlas/salin/ATLAS/VBS_mc/plotting/Ntuple/Plots/All_channels/bins_{opts.bins}/"
else:
    base_dir_plot = f"/exp/atlas/salin/ATLAS/VBS_mc/plotting/Ntuple/Plots/All_channels/linear/bins_{opts.bins}/"
...
all_ops_cat = ["SM","FM0","FS1","FT1","FT5","FT8"]

#all_ops_cat = ["SM"]
processes_all = ["WmZ","WpZ","WmWm","WpWm","WpWp"]
decays_all = ["lvqq"]
...
for decay in decays_all:
    ...
    #print(Cutflow_paths)
        ...
        if opts.Full_op:
            output_plot = f"{base_dir_plot}/full_op/{decay_mode}/{process}/"
        else:   
            output_plot = f"{base_dir_plot}/Fewer_op/{decay_mode}/{process}/"
