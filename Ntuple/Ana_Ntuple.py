...
if not opts.linear:
    base_dir_plot = f"/exp/atlas/salin/ATLAS/VBS_mc/plotting/Ntuple/Plots/Plot_stat_03/bins_{opts.bins}/"
else:
    base_dir_plot = f"/exp/atlas/salin/ATLAS/VBS_mc/plotting/Ntuple/Plots/Plot_stat_03/linear/bins_{opts.bins}/"
...
for process in ["WpZ"]:
    ...
    for decay in ["llqq"]:
        ...
        for op in all_op_plot:
            if op=="SM":
                path = os.path.join(base_dir, f"{process}_{decay}/mc16_13TeV.*.MGPy8EG_aQGCFM0_{op}_1_{process}_{decay}.merge.EVNT.*/DOCUT_YES/Tables/BDT_ntuple_Binary_bis01/ntuple_rivet.root")
                ...
                path = os.path.join(base_dir, f"{process}_{decay}/mc16_13TeV.*.MGPy8EG_aQGC{op}_QUAD_1_{process}_{decay}.merge.EVNT.*/DOCUT_YES/Tables/BDT_ntuple_Binary_bis01/ntuple_rivet.root")
    ...
    if opts.Full_op:
        ...
        output_plot = f"{base_dir_plot}/fewer_few_onlu_op/{process}/"
