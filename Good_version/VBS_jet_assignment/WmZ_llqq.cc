...
namespace Rivet {
    ...
    // // TRUTH INFORMATION !!
        ...
        if(taggingQuarks_.size() == 2){
            ...
            const FourMomentum tag_quarks =  taggingQuarks_[0].mom() + taggingQuarks_[1].mom();
            VBS_Quarks_truth_pT= (tag_quarks).pT();
            VBS_Quarks_truth_eta= (tag_quarks).eta();
            VBS_Quarks_truth_mass= (tag_quarks).mass();
            ...
            Delta_pT_Tagging_jets_VBS_q_truth = abs(tag_quarks.pT() - tag_jets_mom.pT());
            Delta_Mass_Tagging_jets_VBS_q_truth = abs(tag_quarks.mass() - tag_jets_mom.mass());
    ...
    void finalize() {
        if(nsys<1) {
            ...
            std::string VBS_jet_efficiency_file = getOption("OUTDIR") + "/cutflow_merged.txt";
...
/*             const double total_weight_initial = pos_w_sum_initial + neg_w_sum_initial;
    ...
    double VBS_Quarks_truth_pT, VBS_Quarks_truth_eta, VBS_Quarks_truth_mass, VBS_Quarks_truth_Delta_Eta;
    ...
    double Delta_eta_Tagging_Jet1_VBS_q,Delta_eta_Tagging_Jet2_VBS_q;
