...
namespace Rivet {
    ...
    void init() {
        ...
        _tt_truth_mismatch = make_unique<TTree>("Truth", "Rivet_physics");
    ...
    void analyze(const Event& event) {
        ...
        Jets fjets_ = apply<FastJets>(event, "fjets").jetsByPt(Cuts::pT > 200*GeV && Cuts::abseta < 2.0 && Cuts::mass > 50*GeV);
        PseudoJets ljets;
        ...
        for (const PseudoJet &lj : ljets) { 
            if((Jet(lj).pt() > 200*GeV && Jet(lj).abseta() < 2.0 && Jet(lj).mass() > 50*GeV)){
        ...
        if (n_fjets > 0) {
            ...
            if (n_fjets > 1) {
                ...
                //std::cout << "More than one fat jet found" << std::endl;
                //std::cout << "Delta R fjets: " << deltaR(fjets[0], fjets[1]) << std::endl;

                if (Delta_M_fjets > 5) {
                    // Calculate the difference in mass between each fjet and Vhad_mass
                    double deltaMassFjet0 = abs(fjets[0].mass() - Vhad_mass);
                    double deltaMassFjet1 = abs(fjets[1].mass() - Vhad_mass);

                    // Choose the fjet that is closest in mass to Vhad_mass
                    if (deltaMassFjet0 < deltaMassFjet1) {
                        fjet_Vhad = fjets[0];
                    } else {
                        fjet_Vhad = fjets[1];
                    }
            ...
            const FourMomentum fourvec_fjets = fjet_Vhad.mom();

            if ((_docut==1 && (fourvec_fjets.mass() >= _jcuts["m_fjet_WZ"][0] && fourvec_fjets.mass() <= _jcuts["m_fjet_WZ"][1])) || _docut==0) {
                //std::cout << "Pass condition merged for Fjet 0" << std::endl;
                merged = true;
                resolved = false;
                _cutflows_merged.fillnext();
            }
    ...
    // // TRUTH INFORMATION !!
        ...
        if(taggingQuarks_.size() == 2){
            ...
            tag_quarks =  taggingQuarks_[0].mom() + taggingQuarks_[1].mom();
            Tagging_Quarks_pT= (tag_quarks).pT();
            Tagging_Quarks_eta= (tag_quarks).eta();
            Tagging_Quarks_mass= (tag_quarks).mass();
            Tagging_Quarks_Delta_Eta = abs(taggingQuarks_[0].eta() - taggingQuarks_[1].eta());
            ...
            tag_jets_mom = tag_jets[0].mom() + tag_jets[1].mom();
            Tagging_Jets_mass = (tag1_jet + tag2_jet).mass();
            Tagging_Jets_delta_Eta = abs(tag1_jet.eta() - tag2_jet.eta());

            Delta_Eta_Tagging_jets_VBS_q_truth = abs(tag_quarks.eta() - tag_jets_mom.eta());
            Delta_Mass_Tagging_jets_VBS_q_truth = abs(tag_quarks.pT() - tag_jets_mom.pT());
            Delta_pT_Tagging_jets_VBS_q_truth = abs(tag_quarks.mom().mass() - tag_jets_mom.mom().mass());
            ...
            tag_jet1_matched = IsTruthTagJet(all_particles, tag_jet[0], cutof_DR);
            tag_jet2_matched = IsTruthTagJet(all_particles, tag_jet[1], cutof_DR);
            if(tag_jets_matched < 2) {
                tag_jet_mismatched_fromVhad = 0;
                if (tag_jet1_matched == 0) {
                    tag_jet_mismatched_fromVhad=isFromWboson(all_particles, tag_jet[0], cutof_DR);
                }
                if (tag_jet2_matched == 0) {
                    tag_jet_mismatched_fromVhad=isFromWboson(all_particles, tag_jet[1], cutof_DR);
                }
        ...
        if (n_fjets>0 && merged) {
            ...
            const FourMomentum fourvec_fjets_Vlep = fourvec_Vlep + fjet_Vhad.mom();
            FourMomentum fourvec_V = fjet_Vhad.mom();
            // Polarization variable VZ system
            FourMomentum fourvec_VlepVhad = fourvec_Vlep + fjet_Vhad.mom();
            ...
            const FourMomentum fourvec_fjets_full = fourvec_Vlep + fjet_Vhad.mom() + tag1_jet + tag2_jet;
...
/*             const double total_weight_initial = pos_w_sum_initial + neg_w_sum_initial;
    ...
    double Tagging_Quarks_pT, Tagging_Quarks_eta, Tagging_Quarks_mass, Tagging_Quarks_Delta_Eta;
    ...
    std::map<std::string, double*> varMapDouble_truth = {
        {"Tagging_Quarks_Delta_Eta", &Tagging_Quarks_Delta_Eta},
        ...
        {"Tagging_Quarks_pT", &Tagging_Quarks_pT},
        {"Tagging_Quarks_eta", &Tagging_Quarks_eta},
