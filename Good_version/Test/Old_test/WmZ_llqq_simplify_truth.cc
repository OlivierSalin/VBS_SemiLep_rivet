...
/*         const Particle parent_lep1 = GetParent(lep1);
        ...
        for (int i = 0; i < n_jets; i++) {
            ...
            for (int j = i + 1; j < n_jets; j++) {
                ...
                const double delta_eta = abs(i_jet.eta()-j_jet.eta());
                if  (eta_prod < 0.0 && delta_eta> 2.5 && m_tag_jj>max_m_tag_jj){
        ...
        if(taggingQuarks_.size() == 2){
            ...
            Tagging_Quark1_pT = taggingQuarks_[0].pT(), Tagging_Quark1_eta = taggingQuarks_[0].eta(), Tagging_Quark1_mass = taggingQuarks_[0].mom().mass();
            Tagging_Quark2_pT = taggingQuarks_[1].pT(), Tagging_Quark2_eta = taggingQuarks_[1].eta(), Tagging_Quark2_mass = taggingQuarks_[1].mom().mass();
            Tagging_Quarks_mass = (taggingQuarks_[0].mom() + taggingQuarks_[1].mom()).mass();
            Tagging_Jet1_pT = tag_jets[0].pT(), Tagging_Jet1_eta = tag_jets[0].eta(), Tagging_Jet1_mass = tag_jets[0].mom().mass();
            Tagging_Jet2_pT = tag_jets[1].pT(), Tagging_Jet2_eta = tag_jets[1].eta(), Tagging_Jet2_mass = tag_jets[1].mom().mass();
    ...
    double merged_CS_Z_cos_theta,merged_CS_Z_eta,merged_CS_Z_phi,merged_CS_Z_pt;
