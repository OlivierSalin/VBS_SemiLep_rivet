...
namespace Rivet {
    ...
    const Particle GetWboson(const std::vector<Particle>& all_particles){
        ...
        for(const Particle& p : all_particles){
            ...
            if (status == 21) {
                //printf("Particle: PID = %d, pT = %.2f, eta = %.2f, status = %d\n", p.pid(), p.pT(), p.eta(), p_->status());
                for (const Particle& child : p.children()) {
                    ConstGenParticlePtr child_ = child.genParticle();
                    if (child_) {
                        // Print the child particle's PID, pT, and eta
                        //printf("    Child Particle: PID = %d, pT = %.2f, eta = %.2f, status = %d\n", child.pid(), child.pT(), child.eta(), child_->status());

                        // Check if the child particle is a W boson
                        if (child.abspid() == 24) {
                            W_boson = child;
                            W_boson_count++;
                        }

                        // Print the child particles of the current child particle
                        for (const Particle& grandchild : child.children()) {
                            ConstGenParticlePtr grandchild_ = grandchild.genParticle();
                            if (grandchild_) {
                                //printf("  Grandchild Particle: PID = %d, pT = %.2f, eta = %.2f, status = %d\n", grandchild.pid(), grandchild.pT(), grandchild.eta(), grandchild_->status());
                            }
                        }
                    }
                }
...
/*         std::ifstream ana_hist_resolved_file(txt_dir + "/Hists_bis/2lepton_hists_resolved.json");      
        ...
        _tt_merged->Branch("n_jets", &n_jets);
        _tt_merged->Branch("n_EligibleJets", &n_EligibleJets);
        ...
        _tt_aft_lep_cut->Branch("n_EligibleJets", &n_EligibleJets);
        for(auto& var_ : varMapDouble_Vhadboson_truth){
            _tt_aft_lep_cut->Branch(var_.first.c_str(), var_.second);
        }

        _tt_bef_noW_tag_jet = make_unique<TTree>("Tagjet_bef_cut_noW", "Rivet_physics");
        _tt_bef_noW_tag_jet->Branch("EventNumber", &EventNumber);
        _tt_bef_noW_tag_jet->Branch("VBS_event", &VBS_event);
        _tt_bef_noW_tag_jet->Branch("Label", &_label);
        _tt_bef_noW_tag_jet->Branch("n_jets", &n_jets);
        _tt_bef_noW_tag_jet->Branch("n_EligibleJets", &n_EligibleJets);
        _tt_bef_noW_tag_jet->Branch("Eligible_eta_prod", &Eligible_eta_prod);
        _tt_bef_noW_tag_jet->Branch("Eligible_delta_eta", &Eligible_delta_eta);
        ...
        _tt_bef_tag_jet->Branch("n_EligibleJets", &n_EligibleJets);
        _tt_bef_tag_jet->Branch("Eligible_eta_prod", &Eligible_eta_prod);
        _tt_bef_tag_jet->Branch("Eligible_delta_eta", &Eligible_delta_eta);
        for(auto& var_ : varMapDouble_Vhadboson_truth){
        ...
        _tt_truth->Branch("n_EligibleJets", &n_EligibleJets);
        ...
        _tt_truth_mismatch->Branch("n_EligibleJets", &n_EligibleJets);
        for (auto& var_ : varMapDouble_Tagjets_truth) {
        ...
        _tt_truth_mismatch_noVhad->Branch("n_EligibleJets", &n_EligibleJets);
        for (auto& var_ : varMapDouble_Tagjets_truth) {
        ...
        _tt_truth_mismatch_noVhad->Branch("Delta_R_Mismatch_Tagjet_VBS_q", &Delta_R_Mismatch_Tagjet_VBS_q);   
        ...
        _tt_truth_matched->Branch("n_EligibleJets", &n_EligibleJets);
        for (auto& var_ : varMapDouble_Tagjets_truth) {
        ...
        _tt_truth_merged->Branch("n_EligibleJets", &n_EligibleJets);
        for (auto& var_ : varMapDouble_Tagjets_truth) {
        ...
        _cutflows_merged.addCutflow("WpZ_llqq_selections", {"have_two_lep","pt_lep1_2",
                            "n_jets","At_least_one_fjets","fjets_is_W/Z","found_tag_jets","m_tagjets",
                            "Total_Merged_selec",});
    ...
    void analyze(const Event& event) {
        ...
        W_phi = W_bson_.phi();
        if(W_pT > 0) {
            //printf("W boson found with pT: %f\n", W_pT);
            std::vector<Particle> W_quarks_= ValidQuark_W(W_bson_);
            if (W_quarks_.size() >= 2) {
              //printf("W boson found with %d quarks\n", W_quarks_.size());
        ...
        if (n_fjets > 0) {
            ...
            if ((_docut==1 && (fourvec_fjets.mass() >= _jcuts["m_fjet_WZ"][0] && fourvec_fjets.mass() <= _jcuts["m_fjet_WZ"][1])) || _docut==0) {
                Nb_merged_events++;
                //std::cout << "Merged event number: " << Nb_merged_events << std::endl;
                //printf("fjet pT: %.2f, eta: %.2f, mass:  %.2f\n", fjets[0].pT(), fjets[0].eta(),fjets[0].mom().mass());
                //printf("Is W/Z boson: %d\n", IsTrueWboson(all_particles, fjets[0], 0.4, true));
                merged = true;
                resolved = false;
                _cutflows_merged.fillnext();
            }
        ...
        n_EligibleJets = eligibleJets.size();
        ...
        if(merged){
            ...
            fjets_pt_ = fjets[0].pt();
            fjets_mass_ = fjets[0].mass();
            fjets_eta_ = fjets[0].eta();
            Eligible_eta_prod.clear();

            double max_delta_eta = -1.0;
            double eta_prod_max_delta_eta = 0.0;
            if(n_EligibleJets>1){
                for (int i = 0; i < eligibleJets.size(); i++) {
                    for (int j = i + 1; j < eligibleJets.size(); j++) {
                        double delta_eta = abs(eligibleJets[i].eta() - eligibleJets[j].eta());
                        if (delta_eta > max_delta_eta) {
                            max_delta_eta = delta_eta;
                            eta_prod_max_delta_eta = eligibleJets[i].eta() * eligibleJets[j].eta();
                        }
                    }
            }}
            else {
                eta_prod_max_delta_eta = 0.0;
                max_delta_eta = 0.0;
            }


            Eligible_eta_prod.push_back(eta_prod_max_delta_eta);
            Eligible_delta_eta=max_delta_eta;
            _tt_bef_noW_tag_jet->Fill();
                       
        
            if(taggingQuarks_.size() == 2 && W_pT > 0 && merged){
                ...
                VBS_Quarks_Eta_product = taggingQuarks_[0].eta()*taggingQuarks_[1].eta();

                Delta_Eta_fjet_Wboson = abs(fjets[0].eta() - W_bson_.eta());
                Delta_Phi_fjet_Wboson = abs(fjets[0].eta() - W_bson_.eta());
                Delta_pt_fjet_Wboson = abs(fjets[0].pt() - W_bson_.eta());
        ...
        for (int i = 0; i < eligibleJets.size(); i++) {
            ...
            for (int j = i + 1; j < eligibleJets.size(); j++) {   
                ...
                if(merged){
                    const FourMomentum fourvect_VhadZ_ = fjets[0].mom() + fourvec_Vlep;
                    const double delta_phi_FjetZ = abs((i_jet.mom() + j_jet.mom()).phi() - fourvect_VhadZ_.phi());
                    if (eta_prod < 0.0 && m_tag_jj > max_m_tag_jj && delta_phi_FjetZ > 2.8 && delta_phi_FjetZ < 3.4) {
        ...
        if(merged){
            _cutflows_merged.fillnext();
            //printf("merged Tagging jet found\n");
        }
        _cutflows_resolved.fillnext();    
    ...
    // // TRUTH INFORMATION !
        ...
        if(Check_VBS_event(all_particles)){
            if(taggingQuarks_.size() == 2 && W_pT > 0 && merged){
                const FourMomentum fourvector_VhadZ_ = fjets[0].mom() + fourvec_Vlep;
                ...
                if(VBS_Quarks_truth_Delta_Eta > 1.0 && VBS_Quarks_truth_mass > 250 && W_quarks_.size() >= 2){
                    ...
                    Tagging_Jets_FjetZ_Delta_Phi = abs(tag_jets_mom.phi() - fourvector_VhadZ_.phi());
...
/*             const double Centrality_resolved = Centrality(tag_jets, fourvec_Vlep, fourvec_signal_jets);
            ...
            //jet plots
/*             double resolved_n_jets = n_jets;
            double resolved_tagjet1_pt = tag1_jet.pt();
            double resolved_tagjet2_pt = tag2_jet.pt();
            double resolved_tagjets_pt = fourvec_tag_jj.pT();
            double resolved_tagjets_delta_pt = abs(tag1_jet.pt() - tag2_jet.pt());
            double resolved_tagjet1_eta = tag1_jet.eta();
            double resolved_tagjet2_eta = tag2_jet.eta();
            double resolved_tagjets_delta_eta = abs(tag1_jet.eta() - tag2_jet.eta());
            double resolved_tagjet1_phi = tag1_jet.phi();
            double resolved_tagjet2_phi = tag2_jet.phi();
            double resolved_tagjets_m = m_tagjets;
            double resolved_tagjets_eta = fourvec_tag_jj.eta();
            double resolved_tagjets_dy = dy_tagjets;
            double resolved_tagjets_dphi = deltaPhi(tag1_jet, tag2_jet);
            double resolved_n_lepton_stable = nlep;
            double resolved_lepton1_pt = lep1.pT();
            double resolved_lepton2_pt = lep2.pT();
            double resolved_lepton_delta_pt = abs(lep1.pT() - lep2.pT());
            double resolved_lepton1_eta = lep1.eta();
            double resolved_lepton2_eta = lep2.eta();
            double resolved_lepton_delta_eta = abs(lep1.eta() - lep2.eta());
            double resolved_Vlep_mass = m_Vlep;
            double resolved_Vlep_pt = fourvec_Vlep.pT();
            double resolved_Vlep_eta = fourvec_Vlep.eta();
            double resolved_Vlep_phi = fourvec_Vlep.phi();
            double resolved_Vlep_DR_tagjet1 = deltaR(fourvec_Vlep, tag1_jet);
            double resolved_Vlep_DR_tagjet2 = deltaR(fourvec_Vlep, tag2_jet);
            double resolved_Vlep_Dphi_tagjet1 = deltaPhi(fourvec_Vlep, tag1_jet);
            double resolved_Vlep_Dphi_tagjet2 = deltaPhi(fourvec_Vlep, tag2_jet);
            double resolved_Vlep_Deta_tagjet1 = abs(fourvec_Vlep.eta() - tag1_jet.eta());
            double resolved_Vlep_Deta_tagjet2 = abs(fourvec_Vlep.eta() - tag2_jet.eta());
            double resolved_leptons_pids1 = lep1.pid();
            double resolved_leptons_pids2 = lep2.pid();
            double resolved_DR_min_lepton_tagjets1 = std::min(deltaR(lep1, tag1_jet), deltaR(lep2, tag1_jet));
            double resolved_DR_min_lepton_tagjets2 = std::min(deltaR(lep1, tag2_jet), deltaR(lep2, tag2_jet));
            double resolved_DR_min_lepton_sigjets1 = std::min(deltaR(signal_jet1, lep1), deltaR(signal_jet1, lep2));
            double resolved_DR_min_lepton_sigjets2 = std::min(deltaR(signal_jet2, lep1), deltaR(signal_jet2, lep2));
            double resolved_signal_jets_pt1 = signal_jet1.pT();
            double resolved_signal_jets_pt2 = signal_jet2.pT();
            double resolved_signal_jets_eta1 = signal_jet1.eta();
            double resolved_signal_jets_eta2 = signal_jet2.eta();
            double resolved_signal_jets_DeltaEta = abs(signal_jet1.eta() - signal_jet2.eta());
            double resolved_signal_jets_DeltaR = deltaR(signal_jet2, signal_jet1);
            double resolved_signal_jets_DeltaPhi = deltaPhi(signal_jet2, signal_jet1);
            double resolved_signal_jets_mass = signal_mjj;
            double resolved_Vhad_DR_tagjet1 = deltaR(fourvec_Vhad, tag1_jet);
            double resolved_Vhad_DR_tagjet2 = deltaR(fourvec_Vhad, tag2_jet);
            double resolved_Vhad_Dphi_tagjet1 = deltaPhi(fourvec_Vhad, tag1_jet);
            double resolved_Vhad_Dphi_tagjet2 = deltaPhi(fourvec_Vhad, tag2_jet);
            double resolved_Vhad_Deta_tagjet1 = abs(fourvec_Vhad.eta() - tag1_jet.eta());
            double resolved_Vhad_Deta_tagjet2 = abs(fourvec_Vhad.eta() - tag2_jet.eta());
            double resolved_Vhad_DeltaR_Vlep = deltaR(fourvec_Vhad, fourvec_Vlep);
            double resolved_Vhad_DeltaPhi_Vlep = deltaPhi(fourvec_Vhad, fourvec_Vlep);
            double resolved_Vhad_DeltaEta_Vlep = abs(fourvec_Vhad.eta() - fourvec_Vlep.eta());
            double resolved_VlepVhad_mass = fourvec_signal_jets_Vlep.mass();
            double resolved_VlepVhad_pt = fourvec_signal_jets_Vlep.pt();
            double resolved_VlepVhad_eta = fourvec_signal_jets_Vlep.eta();
            double resolved_Full_mass = fourvec_signal_jets_full.mass();
            double resolved_Full_pt = fourvec_signal_jets_full.pt();
            double resolved_Centrality = Centrality_resolved;
            double resolved_CentralityVhad = CentralityVhad_resolved;
            double resolved_CentralityVlep = CentralityVlep_resolved;
            double resolved_CentralityVlepVhad = CentralityVlepVhad_resolved;
            double resolved_ZeppVlep = ZeppVlep_resolved;
            double resolved_ZeppVhad = ZeppVhad_resolved;
            double resolved_ZeppVhadVlep = ZeppVhadVlep_resolved;
            double resolved_Ntrk_tagjets1 = CountChargedTracks(tag_jets[0]);
            double resolved_Ntrk_tagjets2 = CountChargedTracks(tag_jets[1]);
            double resolved_Ntrk_signal_jets1 = CountChargedTracks(sjets_sig[0]);
            double resolved_Ntrk_signal_jets2 = CountChargedTracks(sjets_sig[1]);
            double resolved_mjjj = fourvec_signal_jjj.mass();
            double resolved_DeltaPt_sig_jet2_jet3 = abs(signal_jet2.pT() - sjets_sig[2].mom().pT());
            double resolved_ZeppRes = ZeppRes; */
    ...
    unique_ptr<TTree> _tt_bef_noW_tag_jet;
    ...
    int VBS_event,n_jets, n_EligibleJets;
    ...
    double VBS_Quarks_truth_pT, VBS_Quarks_truth_eta, VBS_Quarks_truth_mass,VBS_Quarks_Eta_product, VBS_Quarks_truth_Delta_Eta, VBS_Quarks_truth_Delta_Phi;
    double Truth_Delta_Eta_VBS_q_VZ, Truth_Delta_Phi_VBS_q_VZ,Delta_Phi_VBS_q_Vlep;

    double Delta_Eta_fjet_Wboson,Delta_Phi_fjet_Wboson,Delta_pt_fjet_Wboson;
    double fjets_pt_,fjets_eta_,fjets_mass_;
    ...
    double Tagging_Jets_VZ_truth_Delta_Phi,Tagging_Jets_FjetZ_Delta_Phi;
    ...
    std::vector<double> Min_DR_Wq_jets, Min_DR_q_Tagjets,Delta_Eta_TaggingQuark1_Wquarks, Eligible_eta_prod,Delta_Eta_TaggingQuark2_Wquarks;
    double Eligible_delta_eta;
    ...
    std::map<std::string, double*> varMapDouble_Tagjets_truth = {
        ...
        {"Tagging_Jets_FjetZ_Delta_Phi", &Tagging_Jets_FjetZ_Delta_Phi},
    ...
    std::map<std::string, double*> varMapDouble_VBS_q_truth = {
        ...
        {"VBS_Quarks_Eta_product", &VBS_Quarks_Eta_product},
        {"Truth_Delta_Eta_VBS_q_VZ", &Truth_Delta_Eta_VBS_q_VZ},
        {"Truth_Delta_Phi_VBS_q_VZ", &Truth_Delta_Phi_VBS_q_VZ},

        {"Delta_Eta_fjet_Wboson", &Delta_Eta_fjet_Wboson},
        {"Delta_Phi_fjet_Wboson", &Delta_Phi_fjet_Wboson},
        {"Delta_pt_fjet_Wboson", &Delta_pt_fjet_Wboson},

        {"fjets_pt_", &fjets_pt_},
        {"fjets_eta_", &fjets_eta_},
        {"fjets_mass_", &fjets_mass_},
